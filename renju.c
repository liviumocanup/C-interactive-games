...
int onGoing, lim=25, altlim=25, py, px;//limits for 65 and 100 are best for a no-board game because when the console is opened in fullscreen, the game doesn't move up and down every time showMe() is called(something that happens for bigger limits).
//The limits could easily be made bigger but in order to play the game properly set 25 and 25 so that the player could move faster than when the limits are 65 and 100 or even 100 100.
char board[100][100];//possibility to set the limits 100 100
...
void checkWin(int pM, int posy, int posx){
    char dir;
    if(pM==1)
        dir='X';
    else dir='O';
    ...
    //horizontally
    for(int i=-4; i<5; i++){
        if(posx+i>=0 && posx+i<altlim){
            if(board[posy][posx+i]==dir){
                win+=1;
            }
            else win=0;
            if(win==5)
    ...
    if(onGoing)
    {
        //vertically
        for(int i=-4; i<5; i++){
            if(posy+i>=0 && posy+i<lim){
                if(board[posy+i][posx]==dir)
                    win+=1;
                else win=0;
                if(win==5)
                {
        ...
        if(onGoing)
        {
            //messed-uplly
            for(int i=-4; i<5; i++){
                if(posy+i>=0 && posy+i<lim && posx+i>=0 && posx+i<altlim){
                    if(board[posy+i][posx+i]==dir)
                        win+=1;
                    else win=0;
                    if(win==5)
                    {
...
void addInput(int pM, int posy, int posx){
    if(pM)
        board[posy][posx]='X';
    else board[posy][posx]='O';
}

void deleteAdmPos(int posy, int posx){

    for(int i=0; i<adm.xlen-1; i+=2){
        if(adm.x[i]==posy && adm.x[i+1]==posx){
            for(int j=i; j<adm.xlen-3; j+=2){
                adm.x[j]=adm.x[j+2];
                adm.x[j+1]=adm.x[j+3];
            }
            adm.xlen-=2;
...
void addAdmPos(int posy, int posx, int addy, int addx){
    adm.x[adm.xlen]=posy+addy;
    adm.xlen++;
    adm.x[adm.xlen]=posx+addx;
    adm.xlen++;
}

void addAdmPosAdj(int posy, int posx){
    for(int i=-1; i<2; i++){
        for(int j=-1; j<2; j++){
            if(i==0 && j==0)
                continue;
            else if(posy+i>=0 && posy+i<lim && posx+j>=0 && posx+j<altlim){
                if(board[posy+i][posx+j]==' '){
                    int already=0;
                    for(int k=0; k<adm.xlen; k+=2){
                        if(adm.x[k]==posy+i && adm.x[k+1]==posx+j){
                            already=1;
                            break;
                        }
                    }if(already==0){
                        addAdmPos(posy, posx, i, j);
                    }
                }
            }
        }
...
int verifyInput(int pM, int posy, int posx){
    if(board[posy][posx]==' '){
        for(int i=-1; i<2; i++){
            for(int j=-1; j<2; j++){
                if(board[posy+i][posx+j]!=' '){
        ...
        if(pM){
            printf("\nNo adjacent pieces, please choose a valid position");
            Sleep(750);
            system("cls");
            showMe();
        }else deleteAdmPos(posy, posx);
        return 0;
    }else {
        if(pM){
            printf("\nNo cheating allowed, be fair and choose a valid position!");
            Sleep(750);
            system("cls");
            showMe();
        }else deleteAdmPos(posy, posx);
...
void startGame(){
    ...
    int playerMove = 1, smallLoop=1;
    int compby=generateRandF()%((2*lim)/5) + (lim/4);
    int compbx=generateRandF()%((2*altlim)/5) + (altlim/4);
    ...
    while(onGoing){
        if(playerMove){
            ...
            int botx;
            do{
                botx=generateRandF()%(adm.xlen/2);
                if(botx%2!=0)
                    botx--;
            }while(!verifyInput(0, adm.x[botx], adm.x[botx+1]));

            addInput(0, adm.x[botx], adm.x[botx+1]);

            checkWin(0, adm.x[botx], adm.x[botx+1]);
            deleteAdmPos(adm.x[botx], adm.x[botx+1]);
            showMe();

            if(onGoing){
                addAdmPosAdj(adm.x[botx], adm.x[botx+1]);
            }
            showMe();
            playerMove=1;
        }
