...
int verify(int n, int m, int p)
{
    ...
    int win=0;
    for(int i=-2; i<3; i++){//horizontally
        if(m+i>=0 && m+i<3){
            if(game[n][m+i]==p)
                win+=1;
            else win=0;
    ...
    for(int i=-2; i<3; i++){//messed-uplly
        if(n+i>=0 && n+i<3 && m+i>=0 && m+i<3){
            if(game[n+i][m+i]==p)
                win+=1;
            else win=0;
            if(win==3)
    ...
    for(int i=-2; i<3; i++){//messed-uplly reversed
        if(n+i>=0 && n+i<3 && m-i>=0 && m-i<3){
            if(game[n+i][m-i]==p)
                win+=1;
            else win=0;
...
void startGame(){
    initRandF(time(NULL));
    int n, m, playerMove, onGoing = 1, temp1, temp2;;

    system("cls");
    printf("Do you want to go first?(0-No, 1-Yes):");
    scanf("%d", &playerMove);
    ...
    for (int i = 0; i < 9 && onGoing; i++)
    {
        if (playerMove == 1)
        {
            printf("\nYour input(nr of row and nr of column as numbered by humans):");
            scanf("%d %d", &n, &m);
            n-=1;
            m-=1;
            while(game[n][m] != ' ')
            {
                printf("\nPlease don't cheat! Type a valid position:");
                scanf("%d %d", &n, &m);
                n-=1;
                m-=1;
            }
        ...
        else if(onGoing)
        {
            do
            {
                temp1 = generateRandF() % 3;
                temp2 = generateRandF() % 3;
            }while(game[temp1][temp2] != ' ');
            game[temp1][temp2]= 'O';
            showme();
            printf("\nBot's input was: %d %d", temp1+1, temp2+1);
            if (verify(temp1, temp2, playerMove) == 1)
            {
