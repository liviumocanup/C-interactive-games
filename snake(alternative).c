...
int onGoing, ate, abusingPower, score;
double targetTime;
...
void keyInput(clock_t *timeS)
{
    if (kbhit())
    {
        ...
        {
            if(tempchkb!=abusingPower && tempchkb!=(abusingPower-32))
                *timeS = clock();
            direction(chkb);
            if(ate==0)
                timingThings(timeS);
...
void startGame(){
    ...
    printf("\n\n\n\tDo you want to play again? Press any key. Otherwise press ESC:");
    char scanned=getch();
    if((int)scanned==27)
